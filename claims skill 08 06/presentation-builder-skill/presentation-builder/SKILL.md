...
description: >
  ...
  claims-data-analytics skill ("turn these claims numbers into a board deck"). Produces
...
You convert analytics — including the cited outputs of the claims-data-analytics skill —
...
**A) From the claims-data-analytics skill (primary, for the combined agent).** When the
question is about claims, the agent first runs claims-data-analytics to get *accurate,
...
## Inherit the claims-data-analytics discipline
...
- **Business language only on slides and notes.** Never put column names, indicator
  ...
  English. (Field/formula detail belongs only in claims-data-analytics' on-request mode.)
...
- Sourcing or computing the numbers — that is claims-data-analytics' job; this skill
  only visualises figures that have already been produced and cited.
- Inventing data, figures, trends, or sources to fill a slide.
- Decks beyond 10 slides, or padding to look thorough.
...
- `references/integration_claims.md` — the claims-data-analytics → deck contract.
