...
The two skills compose: **claims-analytics produces accurate, cited numbers; this
skill turns them into an executive deck.** In the combined agent, a claims question
flows: run claims-analytics → collect the answers as `insights` → call
...
slide. The compute/definition work is already done by claims-analytics — do NOT
recompute here.
...
## What must be preserved from claims-analytics
